...
export function VisionTab({ sensors }) {
  ...
  useEffect(() => {
    if (!camOn || !vidRef.current || !streamRef.current) return;
    vidRef.current.srcObject = streamRef.current;
    vidRef.current.play().catch((e) => console.error("[CAM] play() failed:", e.name, e.message));
  }, [camOn]);
  ...
  const startCam = async () => {
    ...
    try {
      ...
      try {
        ...
        setCamErr("ERROR");
        console.error("[CAM] all attempts failed:", e1.name, e2.name);
        return;
      }
    }
    streamRef.current = stream;
    setCamOn(true); // triggers render of <video>, then useEffect attaches stream
  };
