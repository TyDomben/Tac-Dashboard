    ...
    <div>
      <Panel title="ACOUSTIC DRONE DETECTION" accent={threat ? "#ff4444" : "#00ff88"}>
        <button onClick={on ? stop : start} className="w-full py-3 text-xs tracking-widest mb-4"
          style={{
            border: `1px solid ${on ? (threat ? "#ff444877" : "#00ff8855") : "#00ff8822"}`,
            color: on ? (threat ? "#ff4444" : "#00ff88") : "#00ff8855",
        ...
        {on && (
          <>
              ...
              {Array.from({ length: 28 }, (_, i) => {
                ...
                return (
                  <div key={i} className="flex-1" style={{
                    height: `${h}px`,
                    background: threat
                      ? `rgba(255,68,68,${0.2 + i / 28 * 0.6})`
                      : `rgba(0,255,136,${0.15 + i / 28 * 0.5})`,
                    alignSelf: "flex-end",
                  }} />
                );
              })}
            ...
            <div className="text-xs" style={{ color: "#00ff8822", fontFamily: mono }}>
        ...
        {!on && (
          <div className="text-xs text-center py-6 space-y-1" style={{ color: "#00ff8822", fontFamily: mono }}>
            <div>DETECTS CONSUMER DRONE MOTOR SIGNATURES</div>
            <div style={{ color: "#00ff8818" }}>ALSO RUNS ON IPHONE 5S AS DEDICATED SENSOR</div>
          </div>
        )}
      </Panel>
    </div>
